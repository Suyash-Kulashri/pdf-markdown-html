...
if not api_key:
    st.error("Google API key is missing in .env file. Please provide a valid key.")
    st.stop()
...
# Function to generate PDF with custom styling and bold text handling
def generate_pdf(content, filename="lcm_specs.pdf"):
    try:
        ...
        # Define styles to match LCM500 PDF with custom enhancements
        styles = getSampleStyleSheet()
        title_style = ParagraphStyle(name='Title', parent=styles['Title'], fontSize=16, leading=20, spaceAfter=12, fontName='Helvetica-Bold', textColor=colors.navy)
        section_style = ParagraphStyle(name='Section', parent=styles['Heading1'], fontSize=14, leading=18, spaceAfter=10, fontName='Helvetica-Bold', textColor=colors.black)
        subsection_style = ParagraphStyle(name='SubSection', parent=styles['Heading2'], fontSize=12, leading=16, spaceAfter=8, fontName='Helvetica-Bold', textColor=colors.black)
        normal_style = ParagraphStyle(name='Normal', parent=styles['Normal'], fontSize=10, leading=14, spaceAfter=6, fontName='Helvetica', textColor=colors.black)
        bold_style = ParagraphStyle(name='Bold', parent=styles['Normal'], fontSize=10, leading=14, spaceAfter=6, fontName='Helvetica-Bold', textColor=colors.black)
        ...
        for line in lines:
            ...
            if line.startswith('## '):
                ...
                elements.append(Paragraph(bold_text, subsection_style))
            elif line.startswith('- '):
                if not current_list:
                    current_list = []
                if table_data and table_headers:
...
- Mechanical Drawings (detailed text with weight, dimensions, mounting details, etc.)
...
- Ensure all specifications are as detailed as possible, including numerical values, ranges, conditions, and any relevant notes or assumptions.
- Use markdown formatting with clear section headers (##, ###), bullet lists, and tables. For bold text, use **text** syntax where appropriate (e.g., in Special Features or Ordering Information).
