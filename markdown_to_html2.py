...
Specifications are subject to change without notice. Not responsible for errors or omissions. ©2020 Advanced Energy Industries, Inc. All rights reserved. Advanced Energy®, and AE® are U.S. trademarks of Advanced Energy Industries, Inc."""
    ...
    # Wrap "AT A GLANCE" section with class if present
    at_a_glance_match = re.search(r'<h3>.*?\*\*AT A GLANCE\*\*</h3>', first_page_html, re.DOTALL)
    if at_a_glance_match:
        at_a_glance_start = at_a_glance_match.start()
        at_a_glance_end = first_page_html.find('</h3>', at_a_glance_start) + 5
        at_a_glance_content = first_page_html[at_a_glance_start:at_a_glance_end]
        following_content = first_page_html[at_a_glance_end:]
        first_page_html = (first_page_html[:at_a_glance_start] + 
                          f'<div class="at-a-glance">{at_a_glance_content}' +
                          following_content.replace(at_a_glance_content, '', 1))
    
    # Wrap second and third images in .pmbus-rohs div
    img_pattern = r'<p><img src="([^"]+)" alt="([^"]+)"\s*/></p>'
    img_matches = re.findall(img_pattern, first_page_html)
    if len(img_matches) >= 3:
        second_img = f'<p><img src="{img_matches[1][0]}" alt="{img_matches[1][1]}" /></p>'
        third_img = f'<p><img src="{img_matches[2][0]}" alt="{img_matches[2][1]}" /></p>'
        pmbus_rohs_div = f'<div class="pmbus-rohs">{second_img}{third_img}</div>'
        first_page_html = re.sub(rf'{second_img}.*{third_img}', pmbus_rohs_div, first_page_html, flags=re.DOTALL)
...
if __name__ == "__main__":
    ...
    output_file = "output_with_css2.html"
